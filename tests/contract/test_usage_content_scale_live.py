...
def test_usage_content_scale(live_client, model_name, turn_mode):
    ...
    long_prompt = live_client.chat.completions.create(
        ...
        temperature=0,
        max_tokens=4,
        messages=conversation_messages([{"role": "user", "content": " ".join(["hello"] * 200)}], turn_mode),
    )
    ...
    assert long_completion.usage.completion_tokens >= short_completion.usage.completion_tokens
